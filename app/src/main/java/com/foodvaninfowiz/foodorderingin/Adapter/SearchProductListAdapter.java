...
public class SearchProductListAdapter extends RecyclerView.Adapter<SearchProductListAdapter.MyWishListViewHolder> {
    Context context;
    ...
    private void setVariantData(RecyclerView variantsRecyclerView, int position, List<Variants> variants) {
        ...
        SearchProductVariantsAdapter variantsAdapter = new SearchProductVariantsAdapter(context, variants, position);
    ...
    public void onBindViewHolder(final MyWishListViewHolder holder, final int position) {
      ...
      //  holder.currency.setText("$");
        ...
        if (productList.get(position).getVariants().size() < 2) {
            holder.variantCount.setText("No other variant");
        } else {
            holder.variantCount.setText((productList.get(position).getVariants().size() - 1) + " other variant");
